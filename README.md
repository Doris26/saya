...
| | LingoFlow | 订阅制听写 App(如 Wispr Flow) |
|---|---|---|
...
```bash
git clone https://github.com/yujunzou/lingoflow.git
cd lingoflow
./bundle.sh                          # → dist/LingoFlow.app
open dist/LingoFlow.app              # 菜单栏出现 🎙️ 图标(无 Dock 图标)
```
...
|---|---|---|
| 麦克风 | 录音 | 首次录音弹窗点「允许」 |
| 辅助功能 | 把文字注入到其他 App | 系统设置→隐私与安全性→辅助功能→打开 LingoFlow(注入必需) |
| 通知 | 注入结果/错误提示 | 引导里允许 |
